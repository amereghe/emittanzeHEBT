function ShowFittedEllipsesGrouped(beta,alpha,emiG,planeLabs,labels1,labels2,labels3,myTit,labelsType)
    ...
    for iType=1:length(labelsType)
        for iLab3=1:length(labels3)
            ...
            % general
            sgtitle(sprintf("%s - %s - %s",myTit,labels3(iLab3),LabelMe(labelsType(iType))));
            if ( length(labels1)>1 )
